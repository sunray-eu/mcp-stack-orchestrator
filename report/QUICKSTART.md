...
- Runs local `surrealdb` (`18083`) + Surrealist (`18082`) + two MCP layers:
  ...
  - backend `surrealmcp` on `18084`
- SurrealDB is pinned to `2.3.x` in `infra/versions.env` for current SurrealMCP websocket compatibility.
...
Archon settings sync (automatic):
- `stack_infra.sh up archon` and `stack_infra.sh up full` now push RAG/provider settings to Archon credentials API after health check.
- Managed keys: `LLM_PROVIDER`, `MODEL_CHOICE`, `EMBEDDING_PROVIDER`, `EMBEDDING_MODEL`, `USE_AGENTIC_RAG`, `USE_HYBRID_SEARCH`, `USE_RERANKING`, `USE_CONTEXTUAL_EMBEDDINGS`.
- Override values in `<STACK_ROOT>/.secrets.env` using:
  - `ARCHON_LLM_PROVIDER`
  - `ARCHON_MODEL_CHOICE`
  - `ARCHON_EMBEDDING_PROVIDER`
  - `ARCHON_EMBEDDING_MODEL`
  - `ARCHON_USE_AGENTIC_RAG`
  - `ARCHON_USE_HYBRID_SEARCH`
  - `ARCHON_USE_RERANKING`
  - `ARCHON_USE_CONTEXTUAL_EMBEDDINGS`
- Default model if unset: `gpt-5.2`.
...
Archon file ingestion note:
- `POST /api/documents/upload` rejects some MIME types (`application/octet-stream`, `application/json`).
- For `.toml` / `.json` uploads, set explicit multipart content-type to `text/plain`, or convert to Markdown/text before upload.
...
```bash
task profile:activate PROFILE=core-docs
```
...
- Direct `scripts/*.sh` commands remain available but are considered internal runtime entrypoints.
